...
// For legacy DB records that didn't save the native strings
...
<body>
    ...
    <script>
        ...
        function renderPage() {
            ...
            else if (currentPage === 'grow') { fabButton.style.display = 'none'; content.innerHTML = renderGrowPage(); }
        ...
        function renderGrowPage() {
            return '<h1 class="page-title">Grow</h1>' +
                   '<div class="empty-state" style="margin-top: 40px;">' +
                   '<i class="fas fa-seedling" style="font-size: 3rem; color: var(--success-light); margin-bottom: 16px;"></i>' +
                   '<h3 style="font-size: 1.2rem; color: var(--text-primary-light);">Under Development</h3>' +
                   '<p style="margin-top: 8px; font-size: 0.9rem;">The Grow feature is currently being built. Check back later!</p>' +
                   '</div>';
        }
        ...
        function renderTasksPage() {
            ...
            if (!tasksData || tasksData.length === 0) {
                ...
                tasksData.forEach((task) => {
                    ...
                    // Natively displaying exactly the input strings
                    let displayDate = task.dateIST || task.startDateStr;
                    // Format date as DD-MM-YYYY natively if we have the string YYYY-MM-DD
        ...
        function filterHistoryByMonth(history, year, month) {
            ...
            Object.keys(history).forEach(dateStr => {
                let parts;
                if(dateStr.includes('-') && dateStr.split('-')[0].length === 4) parts = dateStr.split('-'); // YYYY-MM-DD
                else parts = dateStr.split('-').reverse(); // DD-MM-YYYY to YYYY-MM-DD parts
...
app.get('/notes', async (req, res) => {
    try {
        ...
        const istDateObj = getCurrentISTDisplay();
        res.render('index', { currentPage: 'notes', tasks: [], notes: notes.map(n => ({ ...n, createdAtIST: formatLegacyIST(n.createdAt, 'date') + ' ' + formatLegacyIST(n.createdAt, 'time'), updatedAtIST: n.updatedAt ? formatLegacyIST(n.updatedAt, 'date') + ' ' + formatLegacyIST(n.updatedAt, 'time') : '' })), groupedHistory: {}, currentTime: istDateObj.displayTime, currentDate: istDateObj.displayDate });
    } catch (error) { res.status(500).send(error.message); }
});
...
app.get('/history', async (req, res) => {
    try {
        ...
        const istDateObj = getCurrentISTDisplay();
        res.render('index', { currentPage: 'history', tasks: [], notes: [], groupedHistory, currentTime: istDateObj.displayTime, currentDate: istDateObj.displayDate });
    } catch (error) { res.status(500).send(error.message); }
});
...
// NATIVE IST POST CREATION (Anytime Editing Unlocked)
...
app.post('/api/tasks/:taskId/complete', async (req, res) => {
    try {
        ...
        if (task.repeat !== 'none' && task.repeatCount > 0) {
            ...
            // Calculate next Date string for display/storage
