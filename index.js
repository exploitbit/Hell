...
function generateSubtaskId() { return 'sub_' + Date.now().toString(36) + '_' + Math.random().toString(36).substring(2, 6); }
...
bot.command('start', async (ctx) => {
    const keyboard = Markup.inlineKeyboard([
        ...
        [Markup.button.webApp('📊 Grow', WEB_APP_URL + '/progress')],
        [Markup.button.webApp('📜 History', WEB_APP_URL + '/history')]
    ]);
    await ctx.reply('🌟 <b>Global Task Manager</b>\n\nManage your tasks, notes, and growth using the Web App below.', { parse_mode: 'HTML', reply_markup: keyboard.reply_markup });
});
...
// ==========================================
// 🎨 EJS TEMPLATE - WITH INTEGRATED GROW TRACKER
// ==========================================
...
<head>
    ...
    <title>Growth Task Manager</title>
    ...
    <style>
        :root {
            ...
            --border-dark: #334155;
            --accent-dark: #34d399;
            --accent-soft-dark: #064e3b;
            --success-dark: #34d399;
            --warning-dark: #fbbf24;
            --danger-dark: #f87171;
            --hover-dark: #2d3b4f;
            --progress-bg-dark: #334155;
            --ring-today-dark: #34d399;
...
<body>
    ...
    <div class="modal" id="addProgressModal">
        <div class="modal-content">
            ...
            <form id="addProgressForm" onsubmit="submitProgressForm(event)">
                ...
                <button type="submit" class="btn-submit" id="create-progress-btn">Start Growing</button>
            </form>
        </div>
    </div>
    ...
    <div class="modal" id="editProgressModal">
        <div class="modal-content">
            ...
            <form id="editProgressForm" onsubmit="submitEditProgressForm(event)">
                ...
                <button type="submit" class="btn-submit" id="update-progress-btn">Update Growth</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        // Progress colors
        const paletteColors = ['#10b981', '#34d399', '#059669', '#047857', '#065f46', '#064e3b', '#059669', '#047857'];
        ...
        function updateActiveNav() {
            ...
            document.querySelectorAll('.nav-btn').forEach(btn => {
                if (btn.innerText.toLowerCase().includes(currentPage === 'progress' ? 'grow' : currentPage)) btn.classList.add('active');
            });
        }
        ...
        function renderProgressPage() {
            let html = '<h1 class="page-title">Growth Tracker</h1>';
            
            if (!progressData.items || progressData.items.length === 0) {
                html += '<div class="empty-state"><i class="fas fa-seedling" style="font-size:2rem; margin-bottom:10px;"></i><br>No growth tracked yet. Start your journey with the + button.</div>';
                return html;
            }

            // Graphs Panel
            html += '<details class="panel-wrapper" open><summary class="panel-summary"><span>Growth Overview</span><i class="fas fa-chevron-down chevron"></i></summary><div class="panel-body" id="graphs-container">';
            html += renderGraphs();
            html += '</div></details>';

            // Calendar Panel
            html += '<details class="panel-wrapper" open><summary class="panel-summary"><span>Growth Calendar</span><i class="fas fa-chevron-down chevron"></i></summary><div class="panel-body"><div class="month-nav"><button class="nav-btn" onclick="changeMonth(-1)"><i class="fas fa-chevron-left"></i></button><h1 id="month-year-display">' + new Date(currentYear, currentMonth).toLocaleString('default', { month: 'long' }) + ' ' + currentYear + '</h1><button class="nav-btn" onclick="changeMonth(1)"><i class="fas fa-chevron-right"></i></button></div><div class="grid-container"><div class="calendar-grid" id="calendar-grid">';
            ...
            html += '<details class="panel-wrapper" open><summary class="panel-summary"><span>Manage Growth</span><i class="fas fa-chevron-down chevron"></i></summary><div class="panel-body" id="progress-manage-list">';
        ...
        function renderGraphs() {
            if (progressData.items.length === 0) return '<div class="empty-state">No growth added yet.</div>';
        ...
        function renderProgressList() {
            if(progressData.items.length === 0) {
                return '<div class="empty-state"><i class="fas fa-seedling" style="font-size:2rem; margin-bottom:10px;"></i><br>No growth tracked yet. Start your journey with the + button.</div>';
            }
        ...
        function showBubble(cellEl, dateStr) {
            ...
            if(activeItems.length === 0) {
                bubbleHtml += '<div style="font-size:0.75rem; color:var(--text-secondary-light); text-align:center;">No growth active.</div>';
            } else {
        ...
        function updateProgressOnServer() {
            ...
            }).catch(err => console.error('Error saving growth:', err));
        }
        ...
        function submitProgressForm(event) {
            ...
            .then(res => { 
                if(res.ok){ 
                    ...
                    showToast('Growth started!'); 
                    switchPage('progress'); 
                } else throw new Error(''); 
            })
            .catch(err => { showToast('Error starting growth', 'error'); hideLoader(); });
        }
        ...
        function submitEditProgressForm(event) {
            ...
            .then(res => { 
                if(res.ok){ 
                    ...
                    showToast('Growth updated!'); 
                    switchPage('progress'); 
                } else throw new Error(''); 
            })
            .catch(err => { showToast('Error updating growth', 'error'); hideLoader(); });
        }
...
app.post('/api/tasks', async (req, res) => {
    try {
        ...
        // FIXED: Add null checks for all inputs
...
app.post('/api/tasks/:taskId/subtasks', async (req, res) => {
    try {
        if (!req.body.title || !req.body.title.trim()) return res.status(400).send('Empty title');
    ...
    try {
        ...
        await db.collection('tasks').updateOne({ taskId: req.params.taskId, "subtasks.id": req.params.subtaskId }, { 
            $set: { 
                ...
                "subtasks.$.description": req.body.description ? req.body.description.trim() : '' 
            } 
        });
        res.redirect('/tasks');
    } catch (error) { 
...
app.post('/api/progress', async (req, res) => {
    try {
        ...
        if (hasData === 'true') {
            newItem.question = question ? question.trim() : '';
            if (type !== 'boolean') {
...
app.post('/api/progress/:itemId/update', async (req, res) => {
    try {
        ...
        if (hasData === 'true') {
            updatedItem.question = question ? question.trim() : '';
            if (type !== 'boolean') {
        ...
        // FIXED: Remove _id and use replace
        delete progressDoc._id;
        await db.collection('progress').replaceOne({}, progressDoc, { upsert: true });
...
app.post('/api/progress/update', async (req, res) => {
    try {
        ...
        // FIXED: Remove _id before replace
...
async function start() {
    try {
        if (await connectDB()) {
            ...
            await bot.launch();
            console.log('🤖 Bot Started Successfully - With Growth Tracker!');
        } else {
