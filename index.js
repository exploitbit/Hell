            ...
            if (dates.length === 0) { html += '<div class="empty-state"><i class="fas fa-history" style="font-size: 2.5rem;"></i><h3 style="margin-top: 12px; font-size:1.1rem;">No history</h3></div>'; } 
                ...
                dates.forEach(date => {
                    ...
                    tasks.forEach(task => {
                        ...
                        html += '<div style="display: flex; justify-content: space-between; align-items: center; margin-top: 6px;">' + getDayBoxes(task.selectedDays) + '<span style="background: var(--hover-light); padding: 2px 8px; border-radius: 20px; font-size: 0.7rem; color: var(--text-secondary-light); letter-spacing: 0.5px;">' + ((task.dayTypes && task.dayTypes.length) ? task.dayTypes.join('/') : 'WD') + '</span></div>';
